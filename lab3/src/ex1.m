...
function [confusion_matrix, error_rate] = ex1(dataset, random_seed)
...
figure();
hold on;
plot(train_A(:, 1), train_A(:, 2), 'r+');
plot(test_A(:, 1), test_A(:, 2), 'mo');
plot(train_B(:, 1), train_B(:, 2), 'bx');
plot(test_B(:, 1), test_B(:, 2), 'c*');
xlabel('x1');
ylabel('x2');
hold off;
