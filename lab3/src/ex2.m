...
for j=1:max_C
  [LLA, MOG_A] = em_mog(train_A, j, 0);
  [LLB, MOG_B] = em_mog(train_B, j, 0);

  pAC = 0;
  pBC = 0;
  for i=1:size(MOG_A)
    pAC = pAC + MOG_A{i}.PI * mvnpdf(test, MOG_A{i}.MU, MOG_A{i}.SIGMA);
    pBC = pBC + MOG_B{i}.PI * mvnpdf(test, MOG_B{i}.MU, MOG_B{i}.SIGMA);
  end
